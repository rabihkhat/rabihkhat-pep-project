...
public class AccountService {
     ...
     * @return The persisted account if the persistence is successful, null if it was unsuccessfully persisted.
     */
    public Account registerAccount(Account account) {
        Account register = accountDAO.insertAccount(account);
        if (account.getUsername() == null || account.getPassword().length() < 4) {
            return null;
        } else if (register == null ) {
            return null;
        } else if (register.getUsername().isEmpty()) {
            return null;
        } else {
            return register;
        }
    ...
    /**
     * @param account an account object.
     * @return The persisted account if the login is successful, null if it was unsuccessfully logged in.
     */
